...
async function siteHandler(){
  ...
  const inputName = driver.wait(until.elementLocated(By.css('input[name=form_text_27]')));
  await inputName.sendKeys('Злой Тестировщик');
  ...
  await driver.sleep(3000);
  const searchText = driver.wait(until.elementLocated(By.id('title-search-input')));
  await searchText.sendKeys('системы', Key.ENTER);
}
siteHandler()
